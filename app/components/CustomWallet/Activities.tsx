...
const Activities: React.FC = () => {
  ...
  // Filter out any malformed activities
  const validActivities = activities.filter(
    (activity) =>
      activity && (activity.type || activity.description || activity.timeStamp)
  );
  ...
  return (
    <div className="h-full">
      <div className="max-h-[calc(100vh-400px)] sm:max-h-[calc(100vh-350px)] min-h-[200px] overflow-y-auto space-y-3 sm:space-y-4 scrollbar-thin scrollbar-thumb-gray-800 scrollbar-track-transparent">
        <AnimatePresence>
          {hasActivities ? (
            <>
              {validActivities.map((activity, index) => (
                <motion.div
                  ...
                  exit={{ opacity: 0, y: -10 }}
                  transition={{ delay: index * 0.05 }}
                  className="grid grid-cols-[auto_1fr_auto] gap-3 sm:gap-4 px-3 py-4 sm:px-4 sm:py-5 bg-black/60 backdrop-blur-md rounded-lg border border-gray-700 hover:bg-gray-900 transition-all"
                >
                  {/* Icon */}
                  <div
                    className={`w-8 h-8 sm:w-10 sm:h-10 md:w-12 md:h-12 flex items-center justify-center rounded-lg text-white ${
                  ...
                  <div className="text-right w-20 sm:w-24 self-center">
            ...
            <motion.div
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              className="text-center py-6 sm:py-8 h-72 flex justify-center items-center flex-col"
            >
              <Clock className="w-10 h-10 sm:w-12 sm:h-12 text-gray-600 mx-auto mb-2" />
              <p className="text-gray-400 text-sm sm:text-base">
                No activity found
